...
- **`pnpm bench:hash` needs `cargo` for Alloy** -- it compares `ox`, `ox/node`, `ox/wasm`, and `alloy (Rust)` in one table, building `bench/native` with its pinned toolchain. Without `cargo`, the Alloy Keccak256 cells report unavailable while the Ox columns still run. Alloy does not supply the other hashes, which report `n/a`. A full run takes several minutes.
