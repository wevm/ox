...
- **Fuzz budgets are split by cost** -- `pnpm test:fuzz:ci` runs 2k cases per property on Node only, because browsers triple the wall time and their unique risk (native codecs differing between engine versions) is already pinned per pull request by the `*.conformance.ts` suites. `pnpm test:fuzz:soak` adds the browsers and is bounded by `FC_TIME_LIMIT_MS` rather than a run count, since how many cases fit in a budget depends on the property and the runner. The `fuzz` projects raise `testTimeout`: one `test` is thousands of cases, so the unit-test default does not apply.
