...
describe('legacy', () => {
  ...
  describe('errors', () => {
    ...
    test('invalid transaction (some missing)', () => {
      expect(() =>
        TransactionEnvelope.deserialize(Rlp.encode(['0x00', '0x01'])),
      ).toThrowErrorMatchingInlineSnapshot(`
        [InvalidSerializedTransactionError: Invalid serialized transaction of type "legacy" was provided.
    ...
    test('invalid transaction (missing signature)', () => {
      expect(() =>
        TransactionEnvelope.deserialize(
          Rlp.encode(['0x', '0x', '0x', '0x', '0x', '0x', '0x']),
        ),
      ).toThrowErrorMatchingInlineSnapshot(`
        [InvalidSerializedTransactionError: Invalid serialized transaction of type "legacy" was provided.
...
describe('eip2930', () => {
  ...
  describe('errors', () => {
    ...
    test('invalid transaction (some missing)', () => {
      expect(() =>
        TransactionEnvelope.deserialize(
          `0x01${Rlp.encode(['0x00', '0x01']).slice(2)}`,
        ),
      ).toThrowErrorMatchingInlineSnapshot(`
        [InvalidSerializedTransactionError: Invalid serialized transaction of type "eip2930" was provided.
...
describe('eip1559', () => {
  const transaction = TransactionEnvelope.fromEip1559({
    ...
    value: Value.parseEther('1'),
    chainId: 1,
    maxFeePerGas: Value.parseGwei('2'),
    maxPriorityFeePerGas: Value.parseGwei('2'),
  })
