...
describe('static', () => {
  ...
  test('uint', () => {
    ...
    expect(Abi.encode(['uint256'], [69420n])).toBe(
...
describe('seaport', () => {
  test('cancel', () => {
    const cancel = Abi.getItem({
      abi: seaportContractConfig.abi,
      name: 'cancel',
    })
    const data = Abi.encode(cancel.inputs, [orderComponents])
  ...
  test('fulfillAdvancedOrder', () => {
    ...
    const fulfillAdvancedOrder = Abi.getItem({
      abi: seaportContractConfig.abi,
      name: 'fulfillAdvancedOrder',
    })
    const data = Abi.encode(fulfillAdvancedOrder.inputs, [
      {
        denominator: 69n,
        extraData: '0x123123',
        numerator: 420n,
  ...
  test('getCounter', () => {
    const getCounter = Abi.getItem({
      abi: seaportContractConfig.abi,
      name: 'getCounter',
    })
    const data = Abi.encode(getCounter.inputs, [address.vitalik])
    expect(data).toMatchInlineSnapshot(
...
test('invalid type', () => {
  expect(() =>
    Abi.encode([{ name: 'x', type: 'lol' }], [69]),
  ).toThrowErrorMatchingInlineSnapshot(
    `
    [AbiEncodingInvalidTypeError: Type \`lol\` is not a valid encoding type.

    See: https://oxlib.sh/errors#abiencodinginvalidtypeerror]
  `,
  )
})
...
test('invalid address', () => {
  expect(() =>
    Abi.encode([{ name: 'x', type: 'address' }], ['0x111']),
  ).toThrowErrorMatchingInlineSnapshot(`
...
test('invalid bytes', () => {
  expect(() =>
    Abi.encode([{ name: 'x', type: 'bytes8' }], ['0x111']),
  ).toThrowErrorMatchingInlineSnapshot(
    `
