...
test('funding RPC and request codecs retain all fields', () => {
  const rpc = z.encode(Funding.Requirement, requirement)
  expect(rpc.amount).toBe('0x32')
  expect(rpc.slippageBps).toBe('0x0')
  expect(z.decode(Funding.Requirement, rpc)).toEqual(requirement)
...
test('envelope schema preserves funding and rejects invalid slippage', () => {
  ...
  expect(
    z.safeParse(Funding.Domain, { ...requirement, slippageBps: 10001 }).success,
  ).toBe(false)
})
...
test('key schema preserves existing and inline policies', () => {
  for (const fundingPolicy of [
    ...
    {
      admins: [token],
      rules: { maxSlippageBps: 100, sources: { [token]: requirement.sources } },
    },
  ]) {
    const value = {
      address: token,
