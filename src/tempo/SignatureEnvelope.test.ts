...
describe('serialize', () => {
  ...
  describe('keychain', () => {
    ...
    test('behavior: defaults to V2 type identifier when version is omitted', () => {
      ...
      const serialized = SignatureEnvelope.serialize(withoutVersion)
      expect(Hex.slice(serialized, 0, 1)).toBe('0x04')
      expect(serialized).toBe(SignatureEnvelope.serialize(explicitV2))
      expect(serialized).not.toBe(SignatureEnvelope.serialize(explicitV1))
      expect(SignatureEnvelope.deserialize(serialized)).toMatchObject({
        version: 'v2',
      })
    })
  })
