...
export function bytesToBigInt(value: Uint8Array, signed?: boolean): bigint {
  ...
  // BigInt parses hex strings significantly faster than the per-byte loop for
  // > 8-byte inputs in V8/JSC, so route through `bytesToHex`. (For shorter
  // inputs the difference is negligible.)
  const hex = bytesToHex(value)
  const n = hex === '0x' ? 0n : BigInt(hex)
  if (!signed) return n

  const max_unsigned = 1n << (BigInt(value.length) * 8n)
