...
export function assert(
  ...
  if (
    typeof entry.signature.r !== 'bigint' ||
    typeof entry.signature.s !== 'bigint'
  )
    throw new InvalidError('Signature scalars must be bigint.')
  if (
  ...
  const r = entry.signature.r
  const s = entry.signature.s
...
export function fromTuple(tuple: Tuple): FrameSignature {
  ...
  if (scheme === '0x01') {
    ...
    return from({
      ...
      scheme: 'secp256k1',
      signature: {
        r: Hex.toBigInt(Hex.slice(signature, 1, 33)),
        s: Hex.toBigInt(Hex.slice(signature, 33, 65)),
        yParity: Hex.toNumber(Hex.slice(signature, 0, 1)),
      },
    })
  }
  const s = Hex.toBigInt(Hex.slice(signature, 32, 64))
  if (s > p256N / 2n)
  ...
  return from({
    ...
    publicKey: {
      prefix: 4,
      x: Hex.toBigInt(Hex.slice(signature, 64, 96)),
      y: Hex.toBigInt(Hex.slice(signature, 96, 128)),
    },
    scheme: 'p256',
    signature: { r: Hex.toBigInt(Hex.slice(signature, 0, 32)), s },
  })
}
    ...
    switch (entry.scheme) {
      ...
      case 'secp256k1':
        return entry.signature === undefined
          ? '0x'
          : Hex.concat(
              Hex.fromNumber(entry.signature.yParity, { size: 1 }),
              Hex.fromNumber(entry.signature.r, { size: 32 }),
              Hex.fromNumber(entry.signature.s, { size: 32 }),
            )
      case 2:
      case 'p256': {
