...
describe('assert', () => {
  ...
  test('combined gas exceeds 64 bits', () => {
    expect(() =>
      TxEnvelopeEip8141.assert({
        chainId: 1,
        frames: [{ executionGas: 2n ** 64n - 1n }, { stateGas: 1n }],
        sender: accounts[0].address,
      }),
    ).toThrowErrorMatchingInlineSnapshot(
  ...
  test('execution gas exceeds transaction cap', () => {
    expect(() =>
      TxEnvelopeEip8141.assert({
        chainId: 1,
        frames: [{ executionGas: 16_777_216n }],
        sender: accounts[0].address,
      }),
    ).toThrowErrorMatchingInlineSnapshot(
...
describe('getSignPayload', () => {
  test('omitted nested defaults preserve the signing hash and encoding', () => {
    ...
    const explicit = TxEnvelopeEip8141.from({
      ...envelope,
      frames: [Frame.from({ executionGas: 50_000n, mode: 'sender' })],
...
describe('serialize', () => {
  describe('e2e', () => {
    ...
    test('signs, submits, and mines through the envelope and RPC APIs', async () => {
      const target = accounts[1].address
      const before = BigInt(
        await rpc.request({
          method: 'eth_getBalance',
