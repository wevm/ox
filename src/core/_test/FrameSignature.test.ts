...
describe('from', () => {
  ...
  describe('p256', () => {
    test('constructs a structured entry with default payload', () => {
      const signature = { r: 0x01n, s: 0x02n } as const
      const entry = FrameSignature.from({
        publicKey,
        scheme: 'p256',
        signature,
  ...
  describe('secp256k1', () => {
    test('constructs a structured entry with default payload', () => {
      const signature = { r: 0x01n, s: 0x02n, yParity: 1 } as const
    ...
    test('rejects invalid parity', () => {
      expect(() =>
        FrameSignature.from({
          scheme: 'secp256k1',
          signature: { r: 0x01n, s: 0x02n, yParity: 27 },
        }),
      ).toThrow()
    })
  })
...
describe('toTuple', () => {
  ...
  test('normalizes P-256 high-s without mutating input', () => {
    ...
    expect(FrameSignature.toTuple(entry)[3]).toBe(p256Bytes)
    expect(entry.signature.s).toBe(p256.CURVE.n - 2n)
  })
})
...
describe('fromTuple', () => {
  ...
  test('round-trips a real P-256 signature', () => {
    ...
    ) as unknown as FrameSignature.P256
    expect(
      P256.verify({
        payload,
  ...
  test.each([
    ...
    [
      ...
      Hex.concat(
        ...
        Hex.fromNumber(p256.CURVE.n - 2n, { size: 32 }),
...
describe('assert', () => {
  ...
  test.each([0n, secp256k1.CURVE.n, secp256k1.CURVE.n - 1n])(
    'rejects invalid secp256k1 s %s',
    (s) => {
      expect(() =>
        FrameSignature.from({
          scheme: 1,
          signature: { r: 0x01n, s: s, yParity: 0 },
        }),
      ).toThrow()
    },
  )

  test.each([0n, p256.CURVE.n, p256.CURVE.n + 1n])(
...
describe('toRpc', () => {
  ...
  test('preserves explicit payload and signer', () => {
    ...
    expect(entry.signature.r).toBe(1n)
  })
})

describe('structured validation', () => {
  test.each(['invalid', '1', '0x', -1n, 2n ** 256n])(
  ...
  test.each([
    { r: '1', s: 0x02n, yParity: 1 },
    { r: 0x01n, s: '2', yParity: 1 },
    { r: 0x01n, s: 0x02n, yParity: '1' },
    { r: 0x01n, s: 0x02n, yParity: undefined },
  ])('rejects malformed scalars or parity %o', (signature) => {
    const entry = {
      payload: '0x',
      scheme: 'secp256k1',
      signature,
