...
describe('fromNumber', () => {
  ...
  test('behavior: signed, negative, no size', () => {
    // The type requires `size` with `signed`; untyped callers can still omit it,
    // and previously got `0x0` back for a negative `number`.
    expect(() =>
      // @ts-expect-error — `size` is required with `signed`.
...
describe('toBigInt', () => {
  ...
  test('behavior: signed, odd-length hex', () => {
    // Previously threw a `RangeError` from a fractional byte width.
    expect(Hex.toBigInt('0xf', { signed: true })).toBe(15n)
    expect(Hex.toBigInt('0x8', { signed: true })).toBe(8n)
...
describe('toNumber', () => {
  ...
  test('args: signed', () => {
    ...
    expect(Hex.toNumber('0x1a4', { signed: true })).toBe(420)
    expect(
      Hex.toNumber('0xe0', {
        signed: true,
      }),
