...
describe('createCredential', () => {
  ...
  test('behavior: firefox workaround', async () => {
    ...
    expect(publicKey).toMatchInlineSnapshot(`
      {
        "prefix": 4,
        "x": "0x62a31768d44f5eff222f8d70c4cb61abd2840b27d617a7fe8d11b72dd5e86fc1",
        "y": "0x6611bae3f1e2cd38e405153776a7dcb6995b8254a1416ead102a096cb0d80618",
      }
    `)
    ...
    expect(
      WebAuthnP256.verify({
        ...
        signature: {
          r: Hex.fromNumber(
            113791849669138837781667788757552147501372344687388971338283671717487200515057n,
            { size: 32 },
          ),
          s: Hex.fromNumber(
            5390836038580862917709632535460377625750500964730124261384576347991859205326n,
            { size: 32 },
          ),
        },
      }),
...
describe('getSignPayload', () => {
  ...
  test('behavior: WebAuthnP256.sign + P256.verify', () => {
    ...
    const publicKey = PublicKey.from({
      prefix: 4,
      x: Hex.fromNumber(
        60047643624523853691583342123967083322395535713181616683944672258158697774181n,
        { size: 32 },
      ),
      y: Hex.fromNumber(
        49933766055617994255091849813611851260866104136309220262372066694430479376066n,
        { size: 32 },
      ),
    })
    const signature = Signature.from({
      r: Hex.fromNumber(
        51728839035173161960794416403819635570146754182185755610208554815692624617179n,
        { size: 32 },
      ),
      s: Hex.fromNumber(
        42284206126603545568097288331895750698887263268526215297009386324761546383756n,
        { size: 32 },
      ),
    })
...
describe('verify', () => {
  test('default', async () => {
    const publicKey = PublicKey.from({
      prefix: 4,
      x: Hex.fromNumber(
        15325272481743543470187210372131079389379804084126119117911265853867256769440n,
      ),
      y: Hex.fromNumber(
        74947999673872536163854436677160946007685903587557427331495653571111132132212n,
        { size: 32 },
      ),
    })
    const signature = Signature.from({
      r: Hex.fromNumber(
        10330677067519063752777069525326520293658884904426299601620960859195372963151n,
        { size: 32 },
      ),
      s: Hex.fromNumber(
        47017859265388077754498411591757867926785106410894171160067329762716841868244n,
        { size: 32 },
      ),
    })
    const metadata = {
  ...
  test('default', async () => {
    ...
    const signature = Signature.from({
      r: Hex.fromNumber(
        92217130139243395344713469331864871617892993489147165241879962954542036045090n,
        { size: 32 },
      ),
      s: Hex.fromNumber(
        25785067610647358687769954197992440351568013796562547723755309225289815468181n,
        { size: 32 },
      ),
    })
    const metadata = {
