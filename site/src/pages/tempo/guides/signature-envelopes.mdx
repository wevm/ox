...
A P-256 envelope embeds the public key alongside the signature. Sign the payload with
[`P256.sign`](/api/P256/sign), then wrap both values with `SignatureEnvelope.from`.
...
[`WebAuthn.sign`](/api/WebAuthn/sign) returns the authenticator metadata that the
...
// [!code focus:start]
// 2. Sign the payload with the credential.
const { metadata, signature } = await WebAuthn.sign({
  challenge: payload,
  credentialId: credential.id,
})

// 3. Preserve the WebAuthn metadata and public key in the envelope.
const envelope = SignatureEnvelope.from({
  metadata, // [!code hl]
...
// 4. Verify the normalized signature.
const valid = SignatureEnvelope.verify(envelope, {
  payload,
...
// @log: {
// @log:   signature: { r: '0x...', s: '0x...', yParity: 0 },
// @log:   type: 'secp256k1'
// @log: }
// [!code focus:end]
```
...
### Mark WebCrypto P-256 Signatures as Prehashed

Set `prehash: true` when wrapping a WebCrypto P-256 signature. WebCrypto applies SHA-256 before
signing.
