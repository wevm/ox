...
// 1. Set up the root and admin keys.
...
// [!code focus:start]
// 2. Define the account-bound admin authorization.
...
// 2. Define the admin authorization.
const authorization = KeyAuthorization.from({
  account: rootAddress,
  address: adminAddress,
  chainId: 4217n,
...
// [!code focus:start]
// 4. Convert the signed authorization between formats.
...
// 5. Inspect an admin field in each format.
...
### Use Account Bindings Deliberately

Set `account` with `isAdmin: true` for an admin key or `isAdmin: false` for an account-bound
non-admin key. When decoding a wire value that contains `account` without `isAdmin`, Ox preserves
the binding and defaults `isAdmin` to `false`. An `isAdmin` value without `account` is discarded.
