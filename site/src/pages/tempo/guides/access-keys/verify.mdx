...
// 1. Define the trusted account and access key.
const expectedRoot = '0x1111111111111111111111111111111111111111'
const expectedAccessKey = '0x2222222222222222222222222222222222222222'
...
// [!code focus:start]
// 3. Require a V2 keychain signature.
if (
...
// 4. Check the claimed account and recreate its signing payload.
...
// 5. Verify the inner access-key signature.
// [!code hl:start]
...
// 2. Require a V2 keychain signature.
if (
  transaction.signature?.type !== 'keychain' ||
  transaction.signature.version === 'v1'
)
...
// [!code focus:start]
// 3. Recreate the account-bound signing payload.
const payload = TxEnvelopeTempo.getSignPayload(transaction, {
  from: transaction.signature.userAddress,
})

// 4. Extract the claimed account and access key.
