...
with [`MultisigConfig.getAddress`](/tempo/reference/MultisigConfig/getAddress). Supply the recovery
factory configured by your chain. The pinned experimental node has no public-network default.
...
// Replace this development factory with your chain's configured factory.
const factory = '0x7171717171717171717171717171717171717171'
const account = MultisigConfig.getAddress(initialConfig, { factory })
// @log: '0x...'
// [!code focus:end]
```
...
[`SignatureEnvelope.from`](/tempo/reference/SignatureEnvelope/from). Supply both the explicit `account` and the
...
The version-0 config lives in the signature. The first accepted transaction registers its config
commitment. Use that configuration until an owner update stores a new commitment.
...
Use the version-0 config to register the account and until its first owner update. After an update,
