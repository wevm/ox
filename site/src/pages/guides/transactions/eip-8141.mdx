...
support. The example below assumes a funded EOA using the default account behavior;
smart accounts can require different verification frames.
...
The example reads the chain ID and pending nonce from your node. Supply your sender's
private key and a funded recipient address. Choose fees and per-frame budgets for your
network; the values below illustrate the flow on a development network.
...
```ts twoslash
import { TransactionEnvelope, TxEnvelopeEip8141 } from 'ox'
...
const serialized = TransactionEnvelope.serialize(restored)
```

The generic [`TransactionEnvelope`](/api/TransactionEnvelope) API also infers
`eip8141` from `frames`. Its `toTransactionRequest` helper returns a structured
request; use [`TransactionRequest.toRpc`](/api/TransactionRequest/toRpc) before
sending that request through a transport.
...
// The receipt is null until the transaction is mined.
...
<Cards>
  <Card
    ...
    description="Compare transaction types and their fields."
    to="/guides/transactions/envelope-types"
  />
  <Card
    icon="lucide:search"
    title="Parse & Inspect Transactions"
    description="Decode transactions and convert RPC data."
