...
Ox supports six Ethereum transaction envelope types, each with a dedicated module.
They share the same lifecycle functions (`from`, `getSignPayload`, `serialize`), so
...
| [`TxEnvelopeEip8141`](/api/TxEnvelopeEip8141) | [EIP-8141](https://eips.ethereum.org/EIPS/eip-8141) | `0x06` | EIP-1559 fields + optional blob fee    | `frames`, `sender`, `signatures`  |

### Frame transactions (EIP-8141)

Use [`TxEnvelopeEip8141`](/api/TxEnvelopeEip8141) on clients that support frame transactions.
Each frame defines a call, execution mode, approval flags, and execution/state gas budgets.
Frame transactions use a `sender` and a `signatures` list instead of an outer ECDSA signature.
...
Use `toRpc` to convert envelopes to RPC fields. Frame transaction `sender` maps to RPC
`from`; frames and signatures are converted through their respective modules.
Transaction requests use numeric chain IDs, matching the other transaction types.

See [Frame Transactions (EIP-8141)](/guides/transactions/eip-8141) for a complete
build, sign, and send example.
...
no manual `type` bookkeeping. `frames` selects `eip8141` before blob and fee fields.
