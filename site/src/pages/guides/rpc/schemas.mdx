...
const schema = RpcSchema.from<
  ...
  | {
      Request: {
        method: 'wallet_getSecret' // [!code focus]
        params: [id: string] // [!code focus]
      }
      ReturnType: `0x${string}` // [!code focus]
    }
>()

const provider = Provider.from(window.ethereum, { schema }) // [!code focus]
